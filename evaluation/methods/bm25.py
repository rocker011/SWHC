...
from methods.common import BM25ChunkIndex, load_question_data, save_knowledge_results
...
def resolve_context_budget() -> int:
    env_budget = os.getenv("HGRAG_BM25_TOKEN_BUDGET")
    if env_budget:
...
def resolve_chunk_top_k() -> int:
    env_top_k = os.getenv("HGRAG_BM25_TOP_K")
    if env_top_k:
...
async def run(data_source: str, chunk_top_k: int | None = None, token_budget: int | None = None):
    ...
    query_concurrency = int(os.getenv("HGRAG_BM25_QUERY_CONCURRENCY", "8"))
    data, questions = load_question_data(data_source)
    print(
        json.dumps(
            {
                "event": "bm25_chunk_config",
    ...
    async def retrieve_bm25_chunks(question: str) -> str:
        chunk_hits = await asyncio.to_thread(index.query, question, top_k=chunk_top_k)
    ...
    async def query_with_semaphore(question: str) -> str:
        async with sem:
            return await retrieve_bm25_chunks(question)
    ...
    return save_knowledge_results("BM25", data_source, data)
...
def main() -> None:
    ...
    save_path = asyncio.run(run(args.data_source, chunk_top_k=args.chunk_top_k, token_budget=args.token_budget))
